...
def straight_check(hand):
    ...
    if len(indices) != len(set(indices)):
        ...
        return False
    elif sorted([x[:-1] for x in hand]) == ["2", "3", "4", "5", "A"]:
        return True
    elif 5 * min(indices) + 10 == sum(indices):
        return True
    else:
        return False
...
def fullhouse_check(hand):
    ...
    if sorted(list(set(pairs))) == [0, 2, 3]:
        return True
    else:
        return False
...
def hand_evaluator(hand):
    ...
    elif straight_check(hand) is True and flush_check(hand) is True:
        return ("Straight Flush", 9)
    elif [x for x, y in Counter([card[:-1] for card in hand]).items() if y == 4] != []:
        return ("Quads", 8)
    elif fullhouse_check(hand) is True:
        return ("Full House", 7)
    elif flush_check(hand) is True:
        return ("Flush", 6)
    elif straight_check(hand) is True:
        return ("Straight", 5)
    ...
    if hand1_strength > hand2_strength:
        ...
        if sorted(h1_indices) == sorted(h2_indices):
            ...
            if h1_common == h2_common:
                h1_pairs = [
                    x for x,
                    y in Counter(h1_indices).items() if y == 2]
                h2_pairs = [
                    x for x,
                    y in Counter(h2_indices).items() if y == 2]
...
def royalty_scoring(hand, position):
    ...
    # Middle Royalties
        ...
        if hand_evaluator(hand)[1] >= 4:
            return middle_units[
                middle_royalties.index(
                    hand_evaluator(hand)[0])]
        else:
            return 0
...
def final_scoring(hand1, hand2):
    ...
    middle2 = hand2[3:8]
    back2 = hand2[8:]
    user_royalties = royalty_scoring(front, "front") + \
        royalty_scoring(middle, "middle") + \
        royalty_scoring(back, "back")

    ai_royalties = royalty_scoring(front2, "front") + \
        royalty_scoring(middle2, "middle") + \
        royalty_scoring(back2, "back")

    user = 0
    ai = 0
    # Foul Conditions
    if is_foul(front, middle, back) and not is_foul(front2, middle2, back2):
        return (-6 - ai_royalties), (6 + ai_royalties)
    elif is_foul(front, middle, back) and is_foul(front2, middle2, back2):
        return (0, 0)
    elif not is_foul(front, middle, back) and is_foul(front2, middle2, back2):
    ...
    # Standard Hand Comparison
        ...
        if hand_comparison(front, front2):
            user += 1
        elif hand_comparison(front2, front):
            ai += 1
        if hand_comparison(middle, middle2):
            user += 1
        elif hand_comparison(middle2, middle):
            ai += 1
        if hand_comparison(back, back2):
            user += 1
        elif hand_comparison(back2, back):
            ai += 1
...
def is_foul(front, middle, back):
    ...
    if hand_comparison(front, middle) or hand_comparison(front, back)\
            or hand_comparison(middle, back):
        return True
    else:
        return False
...
def is_sweep(front, middle, back, front2, middle2, back2):
    if hand_comparison(front, front2) and hand_comparison(middle, middle2) and hand_comparison(back, back2):
        return "User"
    elif hand_comparison(front2, front) and hand_comparison(middle2, middle) and hand_comparison(back2, back):
        return "AI"
    else:
        return False
...
def ai_starting_hand(hand):
    ...
    # How the computer should place the rest of its range (draws, garbage,
    # etc.)
    front = []
    middle = []
    back = []
    ...
    # If cpu draw trips, place trips + smallest of remainders in the back
        ...
        remainder = [values.index(card[:-1])
                     for card in hand if back.count(card) == 0]
        for card in hand:
    ...
    # Elif the computer has hi card, place the two highest in back, two medium
    # in middle
...
def ai_place_draws(front, middle, back, draw):
    ...
    if len(back) < 5:
        if flush_draw_check(back)[0] and flush_draw_check(back)[1] == draw[0][1]:
            return "back"
...
def greedy_chinese_algorithm(front, middle, back):
    ...
    for candidate in list(combinations(new_deck, 5)):
        if hand_evaluator(candidate)[1] > best or hand_comparison(
                candidate,
                best_hand):
...
def hu_openface(hand_target):
    ...
    while hand_counter < hand_target:
        ...
        while len(cpu_indices) < 5:
            x = randrange(0, 52)
            if cpu_indices.count(x) == 1 or user_indices.count(x) == 1:
                pass
            else:
                cpu_indices.append(x)
        ...
        if hand_counter % 2 == 0:
            ...
            for card in user_hand:
                place = raw_input(
                    "Where do you want to place " + str(card) + " : F, M or B? ")

                print "Your starting hand was " + "".join(user_hand)
                print "-----------------AI HAND-----------------"
                print " ".join(cpu_back) + "\n" + " ".join(cpu_middle) + "\n" + " ".join(cpu_front)

                if place.lower() == "f" and len(front) < 3:
                    front.append(card)
                elif place.lower() == "m" and len(middle) < 5:
                    middle.append(card)
                elif place.lower() == "b" and len(back) < 5:
                    back.append(card)
        ...
        # If button is odd, CPU has the button and user acts first
            ...
            for card in user_hand:
                ...
                # Error Handling
                while place.lower() not in ("f", "m", "b") or (front.count(card) == 0 and middle.count(card) == 0 and back.count(card) == 0):
        ...
        while len(front) + len(middle) + len(back) < 13:
            ...
            remaining_deck = [
                x for x in create_deck() if x not in overall_hand and x not in cpu_overall]
            ...
            if len(front) + len(middle) + len(back) == 12:
                ...
                for card in remaining_deck:
                    ...
                    if is_foul(front, middle, back):
                        potential_fouls += 1

                    if front.count(card) > 0:
                        front.remove(card)
                    elif middle.count(card) > 0:
                        middle.remove(card)
                    elif back.count(card) > 0:
                        back.remove(card)

                print potential_fouls, len(remaining_deck)
                print "You have a %f percent chance of fouling your hand!" % \
                      (100.0 * potential_fouls / len(remaining_deck))

            place = raw_input(
                "Where do you want to place " + str(draw) + " : F, M or B? ")
        ...
        if is_foul(front, middle, back) and not is_foul(cpu_front, cpu_middle, cpu_back):
            print "User fouled!"
            user_hands_fouled += 1
        elif is_foul(front, middle, back) and is_foul(cpu_front, cpu_middle, cpu_back):
            print "Both User and AI fouled!"
            user_hands_fouled += 1
            ai_hands_fouled += 1
        elif not is_foul(front, middle, back) and is_foul(cpu_front, cpu_middle, cpu_back):
            print "AI fouled!"
            ai_hands_fouled += 1
